...
<?cs def:type_link_simple(type) ?>
<a href="<?cs var:toroot ?><?cs var:type.link ?>"><?cs var:type.label ?></a>
<?cs /def ?>
...
def:tag_list(tags) ?><?cs
  each:tag = tags ?><?cs
      ...
      elif:tag.kind == "@inheritDoc" ?><?cs # This is the case when @inheritDoc is in something that doesn't inherit from anything?><?cs
...
def:description(obj) ?><?cs 
  ...
  <div class="jd-tagdata">
    ...
    <dl class="dl-horizontal">
      <?cs each:tag=obj.paramTags ?>
        <dt><?cs if:tag.isTypeParameter ?>&lt;<?cs /if ?><?cs var:tag.name
                ?><?cs if:tag.isTypeParameter ?>&gt;<?cs /if ?></dt>
        <dd><?cs call:tag_list(tag.comment) ?></dd>
      <?cs /each ?>
    </dl>
  </div><?cs 
  /if ?><?cs 
  ...
  <div class="jd-tagdata">
      ...
      <dl class="dl-horizontal"><?cs 
      each:tag=obj.throws ?>  
            <dt><?cs call:type_link(tag.type) ?></dt>
            <dd><?cs call:tag_list(tag.comment) ?></dd><?cs 
      /each ?>
      </dl>
  </div><?cs 
  /if ?><?cs 
...
def:class_link_table(classes) ?>
  <table class="table table-condensed table-striped table-bordered"><tbody><?cs
      each:cl=classes ?>
    <tr class="api apilevel-<?cs var:cl.type.since.key ?>" >
...
def:class_link_list(label, classes) ?><?cs 
  if:subcount(classes) ?>
    <li class="nav-header"><?cs var:label ?></li><?cs
    each cl=classes ?>
      <li class="api apilevel-<?cs var:cl.type.since.key ?>"><?cs call:type_link_simple(cl.type) ?></li><?cs 
    /each ?>
    <?cs 
  /if ?><?cs 
/def ?>
...
def:list(label, classes) ?><?cs 
  if:subcount(classes) ?>
    <li class="nav-header"><?cs var:label ?></li><?cs 
    each:cl=classes ?>
      <li class="<?cs if:class.name == cl.label?>active <?cs /if ?>api apilevel-<?cs var:cl.since.key ?>"><?cs call:type_link_simple(cl) ?></li><?cs 
    /each ?>
    <?cs 
  /if ?><?cs 
/def ?>
...
def:package_link_list(packages) ?><?cs 
  each:pkg=packages ?>
    <li class="<?cs if:(class.package.name == pkg.name) || (package.name == pkg.name)?>active <?cs /if ?>api apilevel-<?cs var:pkg.since.key ?>"><?cs call:package_link(pkg) ?></li><?cs 
  /each ?><?cs
/def ?>
